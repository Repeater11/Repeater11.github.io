...
$.ajax({
  ...
  data: {
    key: 'ORKBZ-FM5YN-WNIFC-SYA5E-YIAUS-XRFNO',  // 这里要写你的KEY!!!
    output: 'jsonp',
  },
  dataType: 'jsonp',
...
function showWelcome() {
  ...
  let dist = getDistance (113.096124, 22.584698, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里换成自己的经纬度
...
function createtime1() {
  var grt = new Date("08/22/2023 00:00:00"); //此处修改你的建站时间或者网站上线时间
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by Repeater %c 你正在访问Repeaterの小家",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| Repeater") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('Repeater的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
function createtime() {
  ...
  // 网站诞生时间
  var grt = new Date("08/22/2023 00:00:00");
...
<p id="swfs">
<a class="swf" id="swf_SiYuanOld" href="javascript:;" rel="noopener external nofollow" style="font-family:'SiYuanOld'!important;color:black" onclick="setFont('SiYuanOld')">思源黑体旧字形</a>
